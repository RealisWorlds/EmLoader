...
export async function pickupNearbyItems(bot) {
    ...
    while (nearestItem) {
        ...
        try {
            ...
            log(bot, `Failed to reach item: ${err.message}`);
            return false;
        }
        await new Promise(resolve => setTimeout(resolve, 200));
...
export async function goToPlayer(bot, username, distance=3) {
    ...
    move.scafoldingBlocks = ['dirt', 'cobblestone', 'stone']; // Specify which blocks to use
...
export async function lookAt(bot, pos) {
    /**
     * Makes the bot look at a given position.
     * @param {Vec3} pos, the position to look at.
     * @returns {Promise<boolean>} true if the look was successful, false otherwise.
     * @example
     * await skills.lookAt(new Vec3(10, 10, 10));
     **/
